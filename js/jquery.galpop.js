...
(function($) {
	...
	var defaultSettings = {
		...
		maxScreen:     80,                  // Percentage of screen size (overrides maxWidth and maxHeight)
	...
	var methods = {
		...
		openBox : function(settings,target,index) {
			...
			if (!target) {
				...
				group = $('[data-galpop-group="'+ rel +'"] img:visible').parent();
		...
		closeBox : function() {
			...
			wrapper.removeClass('complete').fadeOut(50,'swing',function() {
				content.empty();
				info.hide().empty();
		...
		resize : function() {
            ...
            var containerWidth = screenWidth * maxScreen / 100;
            var containerHeight = screenHeight * maxScreen / 100;

            var imgFinalWidth = imageWidth, imgFinalHeight = imageHeight;
            if(imageHeight > containerHeight || imageWidth > containerWidth){
                if(imageHeight / imageWidth > containerHeight / containerWidth){
                    imgFinalHeight = containerHeight;
                	imgFinalWidth = containerHeight / (imageHeight / imageWidth);
                } else {
                    imgFinalWidth = containerWidth;
                    imgFinalHeight = containerWidth / (imageWidth / imageHeight);
				}
			}
			// 130 is reserved for info lable's height
            var heightMargin = containerHeight - 130 - imgFinalHeight, widthMargin = containerWidth - imgFinalWidth;
            container.css({
            	height:     containerHeight,
            	width:      containerWidth
            });
            $(loadedContent.object).css({marginTop: heightMargin/2, marginBottom: heightMargin/2, marginLeft:widthMargin/2, marginRight: widthMargin/2});
		...
		display : function() {
			...
			setTimeout(function() {
				...
				content.append(loadedContent.object).fadeIn(50,'swing',function() {
		...
		moveItem : function(index) {
			...
			info.fadeOut(50,'swing',function() {
				$(this).contents().remove();
			});
			content.fadeOut(50, 'swing', function() {
		...
		infoParse : function() {
			var group = wrapper.data('group');
			var getDescFunc = wrapper.data('getDesc');
			...
			if (group instanceof jQuery) {
				...
				// }
				if(getDescFunc && $.isFunction(getDescFunc)){
					var infoObject = getDescFunc(anchor);
					if (infoObject instanceof jQuery) {
                        infoObject.appendTo(info);
					} else if(typeof infoObject === "string") {
                        $('<p>'+ infoObject +'</p>').appendTo(info);
					}
				}
